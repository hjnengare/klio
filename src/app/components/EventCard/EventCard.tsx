  ...
  return (
    ...
    >
      <Link href={`/event/${event.id}`} className="block group">
        <article className="relative bg-gradient-to-br from-card-bg via-card-bg to-card-bg/95 rounded-lg overflow-hidden cursor-pointer w-[calc(100vw-4rem)] sm:w-[252px] md:w-[270px] lg:w-[288px] flex flex-col border border-white/50 backdrop-blur-md ring-1 ring-white/20 shadow-sm hover:shadow-lg transition-all duration-300">
          <EventBanner
            image={event.image}
            alt={event.alt}
            icon={event.icon}
