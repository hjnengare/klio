...
export function EmailInput({
  ...
  return (
    <div>
      ...
      {/* Validation feedback */}
      {hasError && (
        <p className="text-xs text-orange-600 flex items-center gap-1 mt-1" role="alert" style={{ fontFamily: '"SF Pro New", -apple-system, BlinkMacSystemFont, "SF Pro Display", "SF Pro Text", system-ui, sans-serif", sans-serif', fontWeight: 600 }}>
          <AlertCircle className="w-3 h-3" />
          {error}
        </p>
      )}
      {isValid && (
        <p className="text-xs text-coral flex items-center gap-1 mt-1" role="status" style={{ fontFamily: '"SF Pro New", -apple-system, BlinkMacSystemFont, "SF Pro Display", "SF Pro Text", system-ui, sans-serif", sans-serif', fontWeight: 600 }}>
